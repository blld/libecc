...
static
struct Value revive (struct Parse *parse, struct Value property, struct Value value)
{
...
struct Value object (struct Parse *parse)
{
	...
	if (c.codepoint != '}')
		for (;;)
		{
			...
			if (parse->function)
				value = revive(parse, Value.key(key), value);
...
struct Value array (struct Parse *parse)
{
	...
	for (;;)
	{
		...
		if (parse->function)
		{
			...
			Chars.append(&chars, "%d", object->elementCount);
			key = Chars.endAppend(&chars);
			value = revive(parse, key, value);
		}
